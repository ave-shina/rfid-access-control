...
Buat juga file kredensial broker `infrastructure/passwd` (**tidak di-commit** — hanya berisi hash). Wajib ada sebelum `infra:up`, dan samakan password-nya dengan nilai `MQTT_PASSWORD_*` di `.env`:
...
> Firmware mendukung **auto-discovery broker** (probe IP gateway, lalu scan subnet untuk port `1883`). Nyaman saat development, tetapi discovery mempercayai host mana pun di subnet — untuk penggunaan serius, isi `MQTT_SERVER` di `secrets.h` dengan IP broker secara statis.
...
| `GET` | `/api/logs` | Log akses terbaru (`?limit=`, maks 500) |
| `DELETE` | `/api/logs` | Hapus seluruh log akses |
| `GET` | `/api/users` | Daftar user terdaftar |
| `POST` | `/api/users` | Daftarkan user baru (hash UID + pepper) |
| `PATCH` | `/api/users/{id}` | Aktif/nonaktifkan user (`is_active`) |
...
**Sebelum produksi**, amankan juga: MQTTS/TLS (`:8883`) **plus HMAC per perangkat** — tanpa autentikasi pesan, anti-replay hanya melindungi dari duplikasi tak sengaja, bukan dari penyerang —, autentikasi JWT di semua route, pembatasan role NextAuth pada override, agregasi log (Loki/Grafana), dan cron retensi log. Lihat [`CLAUDE.md`](./CLAUDE.md) untuk checklist pra-produksi lengkap.
