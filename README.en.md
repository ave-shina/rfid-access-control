...
> The firmware performs **broker auto-discovery**: it probes the gateway IP, then scans the local subnet for port `1883`. No hardcoded broker IP needed.
...
| `GET` | `/api/logs` | List access logs (with filters) |
...
**Before production**, also lock down: MQTTS/TLS (`:8883`), JWT auth on all routes, NextAuth role gating on the override, log aggregation (Loki/Grafana), and a log retention cron. See [`CLAUDE.md`](./CLAUDE.md) for the full pre-production checklist.
