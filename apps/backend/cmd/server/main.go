...
func main() {
	...
	api.HandleFunc("/users", func(w http.ResponseWriter, r *http.Request) {
		...
		// Step 1: hash the raw UID (normalized to uppercase to match firmware)
		rawHash := crypto.HashRawUID(req.RFIDUID)

		// Step 2: apply pepper
		pepperedHash := crypto.PepperHash(rawHash, uidPepper)
	...
	api.HandleFunc("/door/override", func(w http.ResponseWriter, r *http.Request) {
		...
		if id, logErr := database.InsertAccessLog(overrideLog); logErr != nil {
