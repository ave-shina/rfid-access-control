...
import (
	...
	"github.com/rfid-access-control/backend/internal/crypto"
...
func NewHandler(broker, clientID, username, password string, database *db.PostgresDB, hub *ws.Hub, pepper string) *Handler {
	...
	opts := pahomqtt.NewClientOptions().
		...
		SetOrderMatters(false). // dispatch each message in its own goroutine; ACK after handler returns (correct QoS 1 semantics)
...
func (h *Handler) pepperHash(uidHash string) string {
	return crypto.PepperHash(uidHash, h.pepper)
}
...
func (h *Handler) processScan(raw []byte) {
	...
	// Input validation (before rate limiting, so malformed messages don't
	// consume a device's rate-limit budget)
	...
	if age < -30*time.Second || age > 30*time.Second {
		log.Warn().Int64("timestamp", payload.Timestamp).Dur("age", age).Msg("timestamp out of range")
		return
	}
	...
	if id, logErr := h.db.InsertAccessLog(accessLog); logErr != nil {
