...
// InsertAccessLog records an access attempt and returns the generated row ID.
// Logging failure must not block door actuation, so callers should handle
// errors but continue.
func (p *PostgresDB) InsertAccessLog(log *models.AccessLog) (int, error) {
	var id int
	err := p.db.QueryRow(
		"INSERT INTO access_logs (rfid_uid_hash, status, action_by, device_id, timestamp) VALUES ($1, $2, $3, $4, $5) RETURNING id",
